...
async def _get_customer_metrics(db: AsyncSession, organization_id: uuid.UUID, args: BaseModel):
...
async def _get_at_risk_customers(db: AsyncSession, organization_id: uuid.UUID, args: BaseModel):
    assert isinstance(args, AtRiskCustomersArgs)
    customers = await analytics_service.get_at_risk_customers(db, organization_id, args.limit)
    return [
        {
            "id": c.id,
            "name": c.name,
...
async def _search_customers(db: AsyncSession, organization_id: uuid.UUID, args: BaseModel):
